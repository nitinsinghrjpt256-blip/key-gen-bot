...
# ==========================================
# ADVANCED KEY GENERATOR BOT
# ==========================================

# 1. Apne Bot Ka Token Yahan Daalein
TOKEN = "MTU0NjkzMjUzMTIwMTU3NzE1Mg.Gw1LQ3.LKBspE2HBR3Fdpjf0iWvkamH87TthJTItTu7Y8"

# 2. Apne Discord Server Ki ID Yahan Daalein (Numeric)
...
@bot.tree.command(name="genkey", description="Generate License Keys for Packages")
...
async def genkey(interaction: discord.Interaction, package: str, days: int = 30, count: int = 1, note: str = ""):
    ...
    try:
        ...
        if data.get("success") and keys:
            ...
            embed.add_field(name="Package ID", value=f"`{package}`", inline=True)
            ...
            if len(keys) == 1:
                ...
                view.add_item(discord.ui.Button(label="Reset HWID", custom_id=f"reset_hwid:{target_key}", style=discord.ButtonStyle.primary))
                view.add_item(discord.ui.Button(label="Ban", custom_id=f"ban_key:{target_key}", style=discord.ButtonStyle.danger))
                view.add_item(discord.ui.Button(label="Unban", custom_id=f"unban_key:{target_key}", style=discord.ButtonStyle.success))
                view.add_item(discord.ui.Button(label="Delete", custom_id=f"delete_key:{target_key}", style=discord.ButtonStyle.secondary))
