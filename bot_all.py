...
async def genkey(
    ...
    if interaction.user.id not in AUTHORIZED_USERS:
        await interaction.response.send_message("❌ Aapke paas keys generate karne ki permission nahi hai!", ephemeral=True)
        return
...
async def managekey(interaction: discord.Interaction, key: str):
    if interaction.user.id not in AUTHORIZED_USERS:
        await interaction.response.send_message("❌ Aapke paas yeh command use karne ki permission nahi hai!", ephemeral=True)
        return
...
TOKEN = os.getenv("DISCORD_TOKEN")
GUILD_ID = 1525181999147388958  # <--- Apne naye server ki Guild ID yahan dalein

# Jinko permission deni hai unki IDs yahan rahegi
AUTHORIZED_USERS = [
    1525179499602509977  # Aapki di gayi User ID
]
...
# Command 1: Generate Package Keys (Restricted to Authorized Users)
@bot.tree.command(name="genkey", description="Generate License Keys for Packages")
@discord.app_commands.choices(package=[
    discord.app_commands.Choice(name="BASIC PANEL", value="e52c1515c53453b85d0d4e87"),
    discord.app_commands.Choice(name="AIMSILENT EXE", value="affc8da8fd5ace99981ab877"),
